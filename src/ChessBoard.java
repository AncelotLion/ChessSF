public class ChessBoard {
    public ChessPiece[][] board = new ChessPiece[8][8]; // поле
    String nowPlayer;
    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                if (board[startLine][startColumn].getSymbol().equals("K") ||  // проверяем позицию для рокировки
                        board[startLine][startColumn].getSymbol().equals("R")) {
                    board[startLine][startColumn].check = false;
                }

                board[endLine][endColumn] = board[startLine][startColumn]; // если фигура может двигаться - перемещаем
                board[startLine][startColumn] = null; // предыдущая клетка обнуляется
    ...
    public boolean castling0() {
        if (nowPlayer.equals("White")) {
            ...
            if (board[0][0].getSymbol().equals("R") && board[0][4].getSymbol().equals("K") && // проверяем короля и ладью
                    board[0][1] == null && board[0][2] == null && board[0][3] == null) {
                if (board[0][0].getColor().equals("White") && board[0][4].getColor().equals("White") &&
                        board[0][0].check && board[0][4].check &&
                        !new King("White").isUnderAttack(this, 0, 2)) { // смотрим, не под атакой ли мы
                    board[0][4] = null;
